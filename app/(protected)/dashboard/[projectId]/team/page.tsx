      ...
      <div className="max-w-6xl mx-auto space-y-8">
        ...
        <div>
          <button
            ...
            className="flex items-center gap-2 text-slate-500 dark:text-gray-400 hover:text-slate-900 dark:hover:text-white mb-6 transition-colors"
          >
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-slate-600 dark:text-gray-400">{project?.title}</p>
            </div>

            {isOwner && (
              <button
                onClick={() => setShowTransferModal(true)}
                className="flex items-center gap-2 px-4 py-2 bg-orange-100 dark:bg-orange-500/20 text-orange-800 dark:text-orange-300 rounded-lg hover:bg-orange-200 dark:hover:bg-orange-500/30 border border-orange-200 dark:border-transparent transition-all font-medium text-sm"
              >
        ...
        {/* Team Members */}
        <div className="bg-white dark:bg-white/[0.02] border border-slate-200 dark:border-white/10 shadow-md dark:shadow-none rounded-2xl p-6">
          <div className="flex items-center gap-2 mb-6">
            <Users className="w-5 h-5 text-purple-600 dark:text-purple-400" />
            <h2 className="text-xl font-semibold text-slate-900 dark:text-white">Team Members</h2>
            <span className="text-sm text-slate-500 dark:text-gray-400">({members.length})</span>
          </div>
          ...
          <div className="space-y-3">
            {members.map((member) => (
              <div
                key={member.id}
                className="flex items-center justify-between p-4 bg-slate-50 dark:bg-white/5 rounded-xl border border-slate-200/80 dark:border-white/10"
              >
                <div className="flex items-center gap-4">
                  ...
                  {member.profiles?.avatar_url ? (
                    <img
                      ...
                      className="w-12 h-12 rounded-full border border-slate-200 dark:border-transparent"
                  ...
                  <div>
                    <div className="flex items-center gap-2">
                      ...
                      {member.role === "owner" && (
                        <Crown className="w-4 h-4 text-amber-500 dark:text-yellow-400" />
                      )}
                    </div>
                    <div className="flex items-center gap-2 text-sm text-slate-500 dark:text-gray-400">
                      ...
                      {member.status === "accepted" ? (
                        ...
                        <span className="flex items-center gap-1 text-amber-600 dark:text-yellow-400 font-medium">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    {member.status === "accepted" && (
                      <select
                        ...
                        className="bg-white dark:bg-white/5 border border-slate-300 dark:border-white/10 rounded-lg px-3 py-2 text-sm text-slate-800 dark:text-gray-300"
                      >
                    ...
                    {member.status === "pending" && (
                      <button
                        ...
                        className="p-2 text-amber-600 dark:text-yellow-400 hover:bg-amber-100 dark:hover:bg-yellow-500/10 rounded-lg transition-all"
        ...
        <div className="bg-white dark:bg-white/[0.02] border border-slate-200 dark:border-white/10 shadow-md dark:shadow-none rounded-2xl p-6">
          ...
          <div className="space-y-2">
            {activityLog.length === 0 ? (
              <p className="text-slate-500 dark:text-gray-400 text-center py-8">No activity yet</p>
            ) : (
              activityLog.map((activity) => (
                <div
                  key={activity.id}
                  className="p-3 bg-slate-50 dark:bg-white/5 border border-slate-200/50 dark:border-transparent rounded-lg text-sm"
                >
